...
;; Create a new loan
(define-public (borrow (collateral-amount uint) (loan-amount uint))
  (begin
    (asserts! (not (var-get paused)) ERR-NOT-AUTHORIZED)
    ...
    (let (
      ...
      (map-set user-deposits 
        tx-sender 
        (- user-deposit collateral-amount)
      )
      ...
      (map-set user-loans
        tx-sender
        (unwrap! (as-max-len? (append (get-user-loans tx-sender) loan-id) u20) ERR-NOT-AUTHORIZED)
      )
...
(define-private (update-loan-interest (loan-id uint))
  (match (get-loan-details loan-id)
    loan-data (let (
      ...
      (map-set protocol-fees 
        current-height ;; Use current height as key
      ...
      loan-data (let (
        ...
        (if is-full-repayment
          ...
          (begin
            ...
            ;; Update total borrowed
            (var-set total-borrowed (- (var-get total-borrowed) (- loan-amount remaining-loan)))
          )
        )
  ...
  (begin
    ...
    (match (get-loan-details loan-id)
      loan-data (let (
        ...
        (map-set protocol-fees 
          current-height
          (+ (default-to u0 (map-get? protocol-fees current-height)) protocol-fee-from-liquidation)
        )
